...
**`make check` is the gate.** It runs `lint`, `links`, `harness-verify` and
`test`, then the tool budget, and it exits non-zero on the first failure. It needs
no network, no credentials and no workspace, so it runs identically on a laptop
and in whatever runner a team already has. Every check in it is a script in
`scripts/` or `harness/scripts/` that can also be run alone.
...
The separate `make check-live` lane exists for the checks that genuinely need a
workspace — a gateway route probe and a live tool-schema measurement. It is not
part of `make check`, because a gate that needs credentials is a gate that gets
skipped.
