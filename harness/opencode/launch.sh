...
#
# Start OpenCode against Unity AI Gateway with this project's rails in place.
#
...
command -v opencode >/dev/null 2>&1 \
  || die "opencode is not on PATH. Install it: docs/01-prerequisites.md"
...
die "there is no environment-only path for OpenCode.

  OpenCode takes a custom provider in `opencode.json` rather than from the environment, so there is nothing for an environment-only path to set. `ug opencode` writes that block; the generated `opencode.json` here carries the permission tiers and the MCP registration, which are the parts that do not depend on the provider.
