...
#
# Start Cursor CLI against Unity AI Gateway with this project's rails in place.
#
...
ROUTE="/ai-gateway/cursor/v1"
PROBE_PATH="/ai-gateway/cursor/v1/models"
...
command -v cursor-agent >/dev/null 2>&1 \
  || die "cursor-agent is not on PATH. Install it: docs/01-prerequisites.md"
...
die "there is no environment-only path for Cursor CLI.

  `ug cursor` configures MCP only; model routing for Cursor is not handled by the launcher, and no environment variable for a custom model endpoint is documented. So this harness can be brought under governed MCP but its model spend is metered wherever your Cursor licence sends it. That is a real answer and the generated launcher says it rather than pretending otherwise.
