...
It writes into your own user-scope harness configuration — for Cursor, the MCP
...
cp harness/cursor/mcp.json         .cursor/mcp.json      # optional: governed MCP
cp harness/cursor/rules/rails.mdc  .cursor/rules/rails.mdc
```

`rails.mdc` has `alwaysApply: true`, so it is in context in every session rather than
retrieved when something looks relevant. For a rule about what must never happen
automatically, that is the only setting worth using.
...
`.cursor/rules/rails.mdc` is the other half of the tier, for the model choosing what to
run. Keep the two in agreement.

What removes the capability rather than describing it is nothing here. The `deny` list in
`cli.json` is still a text match, and there is no working-directory sandbox.

## What this harness cannot express

**`ug cursor` configures MCP and not model routing.** There is no documented
environment variable or config key that points Cursor CLI at a custom model endpoint,
so its tools come under gateway governance and its model spend does not — that is
metered wherever your Cursor licence sends it. If per-team model attribution is the
reason you are here, this harness cannot give it to you today, and no amount of
configuration in this directory changes that. Use one of the other four for work that
