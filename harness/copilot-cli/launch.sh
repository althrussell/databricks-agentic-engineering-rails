...
#
# Start GitHub Copilot CLI against Unity AI Gateway with this project's rails in place.
#
...
PROBE_PATH="/ai-gateway/mlflow/v1/models"
...
command -v copilot >/dev/null 2>&1 \
  || die "copilot is not on PATH. Install it: docs/01-prerequisites.md"
...
if [ "$EXPLAIN" = "1" ]; then
  ...
  if command -v ug >/dev/null 2>&1 && [ "${DAER_NO_UG:-0}" != "1" ]; then
    say "launcher  ug copilot"
  else
    say "launcher  none: ug is required for this harness"
  fi
  say "verified  documented — probed above; not exercised during the build"
  printf '\n'
  exit 0
fi
...
die "there is no environment-only path for GitHub Copilot CLI.

  No documented environment variable points Copilot CLI at a custom model endpoint. Its MCP registration and its tool permissions are both configurable, so the tools come under governance and the model spend does not.
...
PERMISSION_FLAGS="--allow-tool 'write' --allow-tool 'shell(git status)' --allow-tool 'shell(git diff)' --allow-tool 'shell(git log)' --allow-tool 'shell(git show)' --allow-tool 'shell(git add)' --allow-tool 'shell(git commit)' --allow-tool 'shell(git switch)' --allow-tool 'shell(git restore)' --allow-tool 'shell(git stash)' --allow-tool 'shell(git worktree)' --allow-tool 'shell(make check)' --allow-tool 'shell(make docs)' --allow-tool 'shell(make test)' --allow-tool 'shell(make e2e)' --allow-tool 'shell(make doctor)' --allow-tool 'shell(npm run)' --allow-tool 'shell(npm test)' --allow-tool 'shell(npm ci)' --allow-tool 'shell(pytest)' --allow-tool 'shell(uv run)' --allow-tool 'shell(python3 scripts/)' --deny-tool 'shell(git push --force)' --deny-tool 'shell(git push -f)' --deny-tool 'shell(gh pr merge)' --deny-tool 'shell(gh pr ready)' --deny-tool 'shell(gh release create)' --deny-tool 'shell(databricks bundle deploy)' --deny-tool 'shell(databricks bundle destroy)' --deny-tool 'shell(security find-generic-password)'"
say "permissions 22 allowed, 8 denied, as flags on the command line"
printf '\n'
