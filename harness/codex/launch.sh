...
#
# Start Codex CLI against Unity AI Gateway with this project's rails in place.
#
...
ROUTE="/ai-gateway/codex/v1"
PROBE_PATH="/ai-gateway/codex/v1/models"
...
command -v codex >/dev/null 2>&1 \
  || die "codex is not on PATH. Install it: docs/01-prerequisites.md"
...
if [ "$EXPLAIN" = "1" ]; then
  ...
  if command -v ug >/dev/null 2>&1 && [ "${DAER_NO_UG:-0}" != "1" ]; then
    say "launcher  ug codex"
  else
    say "launcher  codex with gateway environment"
  fi
...
say "launcher  codex with gateway environment (no files written)"
printf '\n'
OPENAI_BASE_URL="$HOST$ROUTE" \
