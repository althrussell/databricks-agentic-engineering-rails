...
It writes into your own user-scope harness configuration — for Codex, a custom provider
block in `~/.codex/config.toml`. That is the right thing when you are configuring your
own laptop, which is who this is for, but know that it happens — it is your config, not
this repository's, that changes.

## 5. Install the permission configuration

Codex CLI has no project-scoped configuration file, so this is a merge into your own
`~/.codex/config.toml` rather than a file you commit:

```sh
cat harness/codex/config.toml >> ~/.codex/config.toml
$EDITOR ~/.codex/config.toml     # fill in the two REPLACE-WITH placeholders
```

Then put the instruction file where Codex will read it, in your project root:

```sh
cp harness/codex/AGENTS.md AGENTS.md
```

If you keep the `[mcp_servers.databricks]` block, export a token for it:
...
What removes the capability rather than describing it is
`sandbox_mode = "workspace-write"`, which fences writes to the directory Codex was
started in. That is the strongest fence of the five harnesses here.

## What this harness cannot express

Codex has no pre-execution hook and no per-command rule list, so there is no layer that
inspects a command and refuses it before it runs. The tier is carried by
`approval_policy` plus `AGENTS.md`: you see the command and approve it. That is weaker
than a rule that cannot be talked out of, and it is worth knowing which of the two you
have.

The model route also varies by workspace. On the workspace this pack was built against
the Codex route returned 404 while the Anthropic route returned 200 — see
[`docs/03-gateway-auth.md`](../../docs/03-gateway-auth.md). Check before assuming.
